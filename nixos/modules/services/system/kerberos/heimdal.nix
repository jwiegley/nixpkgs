  ...
  config = mkIf (cfg.enable && kerberos == pkgs.heimdalFull) {
    systemd.services.kadmind = {
      ...
      serviceConfig.ExecStart =
        "${kerberos}/bin/kadmind --config-file=/etc/heimdal-kdc/kdc.conf";
    ...
    systemd.services.kdc = {
      ...
      '';
      serviceConfig.ExecStart =
        "${kerberos}/bin/kdc --config-file=/etc/heimdal-kdc/kdc.conf";
    ...
    systemd.services.kpasswdd = {
      ...
      preStart = ''
        mkdir -m 0755 -p ${stateDir}
      '';
      serviceConfig.ExecStart = "${kerberos}/bin/kpasswdd";
      restartTriggers = [ kdcConfFile ] ++ (attrValues aclFiles);
    };
